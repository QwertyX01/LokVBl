-- ====================================================================
-- VOLLEYBALL LEGENDS - AGGRESSIVE SPORT EDITION (PREMIUM v1.7)
-- Anti-Local-Limit Build — do...end modular wrapper
-- ====================================================================
...
-- ====================================================================
-- SHARED CONTAINER (единая таблица — экономит локалы)
-- ====================================================================
local SHARED = {
    THEME = {
    ...
    Paths        = { logo=nil, brand=nil, banner=nil },
    UI           = {},
    ScreenGui    = nil,
    Tabs         = {},
    TabPages     = {},
...
-- Алиасы (чтобы оригинальный код работал без изменений)
local THEME               = SHARED.THEME
local Config              = SHARED.Config
local CornerElements      = SHARED.CornerElements
local ColorSyncedElements = SHARED.ColorSyncedElements
local SliderRegistry      = SHARED.SliderRegistry
local ToggleRegistry      = SHARED.ToggleRegistry
local Br                  = SHARED.Br
local BallESP             = SHARED.BallESP
local Pred                = SHARED.Pred
local HitboxVisual        = SHARED.HitboxVisual
local MegaHitbox          = SHARED.MegaHitbox
local RangeGuard          = SHARED.RangeGuard
local HitBlocker          = SHARED.HitBlocker
local Tracers             = SHARED.Tracers
local ClothesWiper        = SHARED.ClothesWiper
local Sky                 = SHARED.Sky
...
do
    ...
    for _, gui in ipairs(CoreGui:GetChildren()) do
        if gui.Name == "VL_Menu" or gui.Name == "VL_Panel" or gui.Name == "VL_ESP"
        or gui.Name == "VL_Load" or gui.Name == "RobloxGui" then
...
-- ====================================================================
-- СИНХРОННАЯ ЗАГРУЗКА ЛОГОТИПОВ (без task.spawn)
-- ====================================================================
do
    local function getAssetPath(path)
        if getcustomasset then
            local ok, p = pcall(getcustomasset, path)
            if ok then return p end
        end
        if getgenv and getgenv().getcustomasset then
            local ok, p = pcall(getgenv().getcustomasset, path)
            if ok then return p end
        end
        return nil
    end
    ...
    local ASSETS = {
        { url = "https://i.ibb.co/RkDbPKvG/IMG-20260912-124847.jpg",   file = "vl_logo.png",   key = "logo"   },
        { url = "https://i.ibb.co/WWDZY4jc/14289-removebg-preview.png",file = "vl_brand.png",  key = "brand"  },
        { url = "https://i.ibb.co/tMsVBqwG/IMG-20260828-160933.png",   file = "vl_banner.png", key = "banner" },
    }

    for _, a in ipairs(ASSETS) do
        pcall(function()
            if isfile and writefile then
                if not isfile(a.file) then downloadImage(a.url, a.file) end
                if isfile(a.file) then SHARED.Paths[a.key] = getAssetPath(a.file) end
            end
        end)
    end

    warn("[VL] Logo:   " .. (SHARED.Paths.logo   or "NIL"))
    warn("[VL] Brand:  " .. (SHARED.Paths.brand  or "NIL"))
    warn("[VL] Banner: " .. (SHARED.Paths.banner or "NIL"))

    _G.VL_DeferredLogoLoad = function(imageLabel, url, file, key)
        task.defer(function()
            if SHARED.Paths[key] then
                if imageLabel and imageLabel.Parent then imageLabel.Image = SHARED.Paths[key] end
                return
            end
            local ok, content = pcall(function() return game:HttpGet(url, true) end)
            if ok and content and isfile and writefile then
                pcall(writefile, file, content)
                local p = getAssetPath(file)
                if p then
                    SHARED.Paths[key] = p
                    if imageLabel and imageLabel.Parent then imageLabel.Image = p end
                end
            end
        end)
    end
...
do
    ...
    pcall(function() LoadGui.Parent = ScreenGui.Parent end)
...
-- ====================================================================
-- MAIN PANEL (всё в SHARED.UI)
-- ====================================================================
do
    local UI = SHARED.UI
    local Paths = SHARED.Paths
    ...
    -- === ЛОГОТИПЫ (синхронно, БЕЗ task.spawn) ===
    ...
    LogoImage.Image = Paths.logo or ""    -- синхронно!
    ...
    if not Paths.logo then
        _G.VL_DeferredLogoLoad(LogoImage, "https://i.ibb.co/RkDbPKvG/IMG-20260912-124847.jpg", "vl_logo.png", "logo")
    end
    ...
    -- Сохраняем все ссылки в SHARED.UI
    UI.MainFrame          = MainFrame
    UI.MainScale          = MainScale
    UI.OuterBorder        = OuterBorder
    UI.MainStroke         = MainStroke
    UI.MainStrokeGradient = MainStrokeGradient
    UI.MainGlow           = MainGlow
    UI.PanelHolder        = PanelHolder
    UI.LeftPanel          = LeftPanel
    UI.RightPanel         = RightPanel
    UI.Divider            = Divider
    UI.DividerGradient    = DividerGradient
    UI.LogoImage          = LogoImage
    UI.LogoBadgeGlow      = LogoBadgeGlow
    UI.LogoTitleGradient  = LogoTitleGradient
    UI.LogoSubGradient    = LogoSubGradient
    UI.PageHeader         = PageHeader
    UI.AccentBar          = AccentBar
    UI.AccentGradient     = AccentGradient
    UI.PageIndex          = PageIndex
    UI.PageTitle          = PageTitle
    UI.onlineDot          = onlineDot
    UI.HeaderBaseLine     = HeaderBaseLine
    UI.HeaderRunner       = HeaderRunner
    UI.RunnerGradient     = RunnerGradient
    UI.HeaderPulse        = HeaderPulse

    -- CreateBracket (4 угла)
...
do
    ...
    local TabPages = SHARED.TabPages
    local TabNames = {"Main", "Visuals", "Combat", "Sky", "Settings"}
    local TabIndexes = { "01", "02", "03", "04", "05" }
    local TabBaseY = SHARED.TabBaseY
    local TabHeight = SHARED.TabHeight
    local TabSpacing = SHARED.TabSpacing
    ...
    for i, name in ipairs(TabNames) do
        ...
        tab.MouseEnter:Connect(function()
            if not Tabs[name] or not Tabs[name].IsActive then
        ...
        tab.MouseButton1Click:Connect(function()
            ...
            if Tabs[name].IsActive then
                ...
                SHARED.ActiveTab = nil
                TweenService:Create(UI.PageTitle, TweenInfo.new(0.2), {TextTransparency = 1}):Play()
                TweenService:Create(UI.PageIndex, TweenInfo.new(0.2), {TextTransparency = 1}):Play()
                TweenService:Create(UI.AccentBar, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
                TweenService:Create(UI.HeaderBaseLine, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
                TweenService:Create(UI.HeaderRunner, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
                TweenService:Create(UI.HeaderPulse, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
                task.delay(0.2, function() UI.PageHeader.Visible = false end)
                return
            end

            Tabs[name].IsActive = true
            SHARED.ActiveTab = name
            UI.PageHeader.Visible = true
            UI.PageTitle.Text = name:upper()
            UI.PageTitle.TextTransparency = 1
            UI.PageIndex.Text = TabIndexes[i]
            UI.PageIndex.TextTransparency = 1
            UI.AccentBar.BackgroundTransparency = 1
            UI.HeaderBaseLine.BackgroundTransparency = 0.7
            UI.HeaderRunner.BackgroundTransparency = 0
            UI.HeaderPulse.BackgroundTransparency = 0.6
            TweenService:Create(UI.PageTitle, TweenInfo.new(0.25), {TextTransparency = 0}):Play()
            TweenService:Create(UI.PageIndex, TweenInfo.new(0.25), {TextTransparency = 0}):Play()
            TweenService:Create(UI.AccentBar, TweenInfo.new(0.25), {BackgroundTransparency = 0}):Play()
...
-- ====================================================================
-- MAIN PAGE (banner, greet, statTiles, avatar, brand, FPS, dots, scanline)
-- ====================================================================
do
    local UI = SHARED.UI
    local Paths = SHARED.Paths
    local TabPages = SHARED.TabPages
    local statTiles = SHARED.StatTiles
    ...
    bannerImage.Image = Paths.banner or ""
    ...
    if not Paths.banner then
        _G.VL_DeferredLogoLoad(bannerImage, "https://i.ibb.co/tMsVBqwG/IMG-20260828-160933.png", "vl_banner.png", "banner")
    end
    ...
    DragHandle.Parent = UI.MainFrame
    ...
    DragButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            dragStartFrame = UI.MainFrame.Position
        end
    end)
    ...
    AvatarFrame.Parent = UI.LeftPanel
    ...
    PlayerName.Parent = UI.LeftPanel
    ...
    BrandFrame.Parent = UI.LeftPanel
    ...
    BrandImage.Image = Paths.brand or ""
    ...
    if not Paths.brand then
        _G.VL_DeferredLogoLoad(BrandImage, "https://i.ibb.co/WWDZY4jc/14289-removebg-preview.png", "vl_brand.png", "brand")
    end
    ...
    DotContainer.Parent = UI.LeftPanel
    ...
    local function RebuildDots()
        ...
        local w = UI.LeftPanel.AbsoluteSize.X
        local h = UI.LeftPanel.AbsoluteSize.Y
    ...
    local function UpdateDots()
            -- (продолжение блока Main Page из Части 2)
    local function UpdateDots()
        if not Config.FlyingDotsEnabled then return end
        local w = UI.LeftPanel.AbsoluteSize.X
        local h = UI.LeftPanel.AbsoluteSize.Y
    ...
    -- Сохранение ссылок в UI
    UI.bannerStroke       = bannerStroke
    UI.bannerGlow         = bannerGlow
    UI.bannerImage        = bannerImage
    UI.splitLine          = splitLine
    UI.splitGrad          = splitGrad
    UI.greetTitleGradient = greetTitleGradient
    UI.greetBodyGradient  = greetBodyGradient
    UI.statusSectionLine  = statusSectionLine
    UI.statusSectionLabel = statusSectionLabel
    UI.DragCursor         = DragCursor
    UI.AvatarStroke       = AvatarStroke
    UI.AvatarGlow         = AvatarGlow
    UI.PlayerTag          = PlayerTag
    UI.BrandImage         = BrandImage
    UI.FpsFrame           = FpsFrame
    UI.FpsLabel           = FpsLabel
    UI.FpsGradient        = FpsGradient
    UI.DotContainer       = DotContainer
    UI.StatusDot          = StatusDot
    UI.DotGradient        = DotGradient
    UI.ScanLine           = ScanLine
    UI.ScanGradient       = ScanGradient
    UI.RebuildDots        = RebuildDots

    -- ============ УНИВЕРСАЛЬНЫЙ ЛУП ГРАДИЕНТОВ (1 корутина вместо 25) ============
    task.spawn(function()
        while ScreenGui.Parent do
            local offset = math.sin(tick() * 1.2)
            if UI.MainStrokeGradient then UI.MainStrokeGradient.Offset = Vector2.new(offset, 0) end
            if UI.DividerGradient then UI.DividerGradient.Offset = Vector2.new(0, offset) end
            if UI.AccentGradient then UI.AccentGradient.Offset = Vector2.new(offset, 0) end
            if UI.LogoTitleGradient then UI.LogoTitleGradient.Offset = Vector2.new(offset, 0) end
            if UI.LogoSubGradient then UI.LogoSubGradient.Offset = Vector2.new(offset, 0) end
            if UI.FpsGradient then UI.FpsGradient.Offset = Vector2.new(offset, 0) end
            if UI.DotGradient then UI.DotGradient.Offset = Vector2.new(offset, 0) end
            if UI.greetTitleGradient then UI.greetTitleGradient.Offset = Vector2.new(offset, 0) end
            if UI.greetBodyGradient then UI.greetBodyGradient.Offset = Vector2.new(offset, 0) end
            if Br.TL_hg then Br.TL_hg.Offset = Vector2.new(offset, 0) end
            if Br.TR_hg then Br.TR_hg.Offset = Vector2.new(offset, 0) end
            if Br.BL_hg then Br.BL_hg.Offset = Vector2.new(offset, 0) end
            if Br.BR_hg then Br.BR_hg.Offset = Vector2.new(offset, 0) end
            if Br.TL_vg then Br.TL_vg.Offset = Vector2.new(0, offset) end
            if Br.TR_vg then Br.TR_vg.Offset = Vector2.new(0, offset) end
            if Br.BL_vg then Br.BL_vg.Offset = Vector2.new(0, offset) end
            if Br.BR_vg then Br.BR_vg.Offset = Vector2.new(0, offset) end
            task.wait(0.03)
        end
    end)

    -- HeaderRunner (позиционная)
    task.spawn(function()
        while ScreenGui.Parent do
            UI.HeaderRunner.Position = UDim2.new(-0.3, 0, 0, 45)
            local t = TweenService:Create(UI.HeaderRunner, TweenInfo.new(1.2, Enum.EasingStyle.Linear), {Position = UDim2.new(1, 0, 0, 45)})
            t:Play()
            t.Completed:Wait()
            task.wait(0.3)
        end
    ...
    task.spawn(function()
        while ScreenGui.Parent do
            TweenService:Create(UI.LogoBadgeGlow, TweenInfo.new(0.8), {Transparency = 0.3}):Play()
            TweenService:Create(UI.MainGlow, TweenInfo.new(0.8), {Transparency = 0.6}):Play()
            TweenService:Create(UI.AvatarGlow, TweenInfo.new(0.8), {Transparency = 0.75}):Play()
            TweenService:Create(UI.HeaderPulse, TweenInfo.new(0.6), {BackgroundTransparency = 0.15}):Play()
            task.wait(0.8)
            TweenService:Create(UI.LogoBadgeGlow, TweenInfo.new(0.8), {Transparency = 0.75}):Play()
            TweenService:Create(UI.MainGlow, TweenInfo.new(0.8), {Transparency = 0.95}):Play()
            TweenService:Create(UI.AvatarGlow, TweenInfo.new(0.8), {Transparency = 0.95}):Play()
            TweenService:Create(UI.HeaderPulse, TweenInfo.new(0.6), {BackgroundTransparency = 0.85}):Play()
            task.wait(0.8)
        end
    end)
    ...
    task.spawn(function()
        while ScreenGui.Parent do
            TweenService:Create(UI.onlineDot, TweenInfo.new(0.9, Enum.EasingStyle.Sine), {BackgroundTransparency = 0.6, Size = UDim2.new(0, 5, 0, 5)}):Play()
            task.wait(0.9)
            TweenService:Create(UI.onlineDot, TweenInfo.new(0.9, Enum.EasingStyle.Sine), {BackgroundTransparency = 0, Size = UDim2.new(0, 6, 0, 6)}):Play()
            task.wait(0.9)
        end
    end)
...
-- ====================================================================
-- HELPERS (CreateSection / CreateToggle / CreateSlider)
-- ====================================================================
do
    function SHARED.CreateSection(parent, title, yPos, color)
    ...
    function SHARED.CreateToggle(parent, name, descText, yPos, default, callback)
    ...
    function SHARED.CreateSlider(parent, name, descText, yPos, minVal, maxVal, default, suffix, callback)
        local frame = Instance.new("Frame")
    ...
    -- Прокидываем в глобальные переменные, чтобы секции ниже вызывали без SHARED.
    CreateSection = SHARED.CreateSection
    CreateToggle  = SHARED.CreateToggle
    CreateSlider  = SHARED.CreateSlider
end

-- ====================================================================
-- BALL / HITBOX / TRACER / SKY HELPERS
-- ====================================================================
...
local function _WipeCharacter(char)
    ...
    if sg then pcall(function() sg.Graphic = "" end) end
...
local function _CreateBallESP(ball)
    ...
    BallESP.highlight.Parent = ScreenGui
...
local function ExpandAllHitboxTemplates()
    ...
    local count = 0
    local function processFolder(root)
        if not root then return end
        for _, child in ipairs(root:GetChildren()) do
            local assemblies = child:FindFirstChild("Assemblies")
            if assemblies then
    ...
    processFolder(HitboxesNew:FindFirstChild("Default"))
    processFolder(HitboxesNew:FindFirstChild("BySpecial"))
    return count
end
...
local function _TracerGetHead(player)
    ...
    for _, obj in ipairs(char:GetDescendants()) do
        if obj:IsA("BasePart") and string.lower(obj.Name) == "head" then return obj end
    end
...
local function _TracerUpdate()
    if not Tracers.Enabled then
        for player, _ in pairs(Tracers.Active) do _TracerDestroy(player) end
    ...
    for player, _ in pairs(Tracers.Active) do
        if not player.Parent then _TracerDestroy(player) end
    end
end
...
local function _SkyAtmosh()
    ...
    task.spawn(function()
        ...
        while sky.Parent do
            ...
            pcall(function() sky.SkyboxOrientation = Vector3.new(5, angle, 0) end)
            task.wait(0.05)
        end
    end)
    Sky.Current = "Atmosh"
...
do
    local TabPages = SHARED.TabPages
    local skyButtons = SHARED.SkyButtons
    ...
    local function CreateSkyButton(preset, yPos)
        local frame = Instance.new("Frame")
...
do
    ...
    CreateToggle(settingsPage, "Flying Dots", "Floating particles in left panel", 40, Config.FlyingDotsEnabled, function(v)
        ...
        if UI.RebuildDots then UI.RebuildDots() end
    end)
    ...
    CreateToggle(settingsPage, "Scan Line", "Moving light animation", 150, Config.ScanLineEnabled, function(v)
        ...
        if UI.ScanLine then UI.ScanLine.Visible = v end
    end)
    ...
    CreateSlider(settingsPage, "Menu Scale", "Resize the whole menu", 360, 70, 130, 100, "%", function(v)
        Config.MenuScale = v
        TweenService:Create(UI.MainScale, TweenInfo.new(0.15), {Scale = v / 100}):Play()
    end)
    ...
    CreateSlider(settingsPage, "Corner Radius", "Round corners", 420, 0, 16, 8, "px", function(v)
        ...
        for _, el in ipairs(CornerElements) do
            ...
            local offset = (parent == UI.PanelHolder) and -2 or 0
    ...
    local pickerDotOutline = Instance.new("UIStroke", pickerDot)
    pickerDotOutline.Thickness = 1.5
    pickerDotOutline.Color = Color3.fromRGB(0, 0, 0)
    ...
    local function ApplyAccentColor(color)
        ...
        UI.MainStroke.Color = newAccent
        if UI.MainStrokeGradient then
            UI.MainStrokeGradient.Color = ColorSequence.new({
        ...
        UI.MainGlow.Color = newGlow
        UI.LogoBadgeGlow.Color = newGlow
        UI.AvatarStroke.Color = newHot
        UI.AvatarGlow.Color = newGlow
        UI.Divider.BackgroundColor3 = newAccent
        UI.DividerGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.25, newHot),
            ColorSequenceKeypoint.new(0.5, newGlow),
            ColorSequenceKeypoint.new(0.75, newHot),
        ...
        UI.AccentBar.BackgroundColor3 = newAccent
        UI.AccentGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.25, newHot),
            ColorSequenceKeypoint.new(0.5, newGlow),
            ColorSequenceKeypoint.new(0.75, newHot),
        ...
        UI.HeaderBaseLine.BackgroundColor3 = newDark
        UI.HeaderRunner.BackgroundColor3 = newHot
        UI.HeaderPulse.BackgroundColor3 = newGlow
        UI.ScanLine.BackgroundColor3 = newHot
        UI.StatusDot.BackgroundColor3 = newHot
        UI.PlayerTag.TextColor3 = newHot
        UI.DragCursor.ImageColor3 = newGlow
        paletteStroke.Color = newDark
        UI.bannerStroke.Color = newAccent
        UI.bannerGlow.Color = newGlow
        UI.statusSectionLine.BackgroundColor3 = newHot
        UI.statusSectionLabel.TextColor3 = newHot
        UI.splitLine.BackgroundColor3 = newAccent

        UI.greetTitleGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newGlow),
            ColorSequenceKeypoint.new(0.5, THEME.TEXT_HI),
            ColorSequenceKeypoint.new(1, newHot),
        })
        UI.greetBodyGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newGlow),
            ColorSequenceKeypoint.new(0.4, THEME.TEXT_HI),
            ColorSequenceKeypoint.new(0.7, newHot),
            ColorSequenceKeypoint.new(1, newDark),
        })
        UI.splitGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
        ...
        UI.LogoTitleGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.5, THEME.TEXT_HI),
            ColorSequenceKeypoint.new(1, newDark),
        })
        UI.LogoSubGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.5, newHot),
            ColorSequenceKeypoint.new(1, newDark),
        })
        UI.FpsGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.5, newHot),
            ColorSequenceKeypoint.new(1, newDark),
        })
        UI.DotGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, newDark),
            ColorSequenceKeypoint.new(0.5, newHot),
            ColorSequenceKeypoint.new(1, newDark),
        })
        UI.ScanGradient.Color = ColorSequence.new({
        ...
        for _, tData in pairs(SHARED.Tabs) do
    ...
    MakeActionButton("Reset Settings", 785, Color3.fromRGB(255, 180, 100), function()
        for name, fn in pairs(ToggleRegistry) do
            if name == "Flying Dots" or name == "Sounds" or name == "Scan Line" then
                fn(true, true)
            else
                fn(false, true)
            end
        ...
        UI.FpsFrame.Visible = false
        TweenService:Create(UI.MainScale, TweenInfo.new(0.2), {Scale = 1}):Play()
        if UI.RebuildDots then UI.RebuildDots() end
        if UI.ScanLine then UI.ScanLine.Visible = true end
        ApplyAccentColor(Color3.fromRGB(180, 80, 255))
        pickerDot.Position = UDim2.new(0.5, 0, 0.5, 0)
        hexLabel.Text = "#B450FF"
    end)
...
-- ====================================================================
-- DROP-IN + KEYBIND + ФИНАЛЬНАЯ АКТИВАЦИЯ MAIN TAB
-- ====================================================================
do
    local UI = SHARED.UI
    local Tabs = SHARED.Tabs
    ...
    task.spawn(function()
        task.wait(1.6)
        local dropTween = TweenService:Create(UI.MainFrame,
    ...
    UI.PageHeader.Visible = true
    UI.PageTitle.Text = "MAIN"
    UI.PageTitle.TextTransparency = 0
    UI.PageIndex.Text = "01"
    UI.PageIndex.TextTransparency = 0
    UI.AccentBar.BackgroundTransparency = 0
    UI.HeaderBaseLine.BackgroundTransparency = 0.7
    UI.HeaderRunner.BackgroundTransparency = 0
    UI.HeaderPulse.BackgroundTransparency = 0.6

    print("[VL] Loaded v1.7 — modular wrapper, no local-limit crash")
    end
